...
export function checkHttpStatus(prefetched) {
  const code = prefetched.status;
  if (!code) {
    return { code: null, message: '접속 실패', pass: false, location: null, details: [] };
  }

  const message = getStatusMessage(code);
  const pass = code >= 200 && code < 400;
  const location = prefetched.headers["location"] || null;

  const details = [];
  if (code >= 200 && code < 300) {
    details.push({ type: "info", text: `HTTP ${code} - ${message}` });
  } else if (code >= 300 && code < 400) {
    details.push({ type: "info", text: `HTTP ${code} - ${message}` });
  } else {
    details.push({ type: "warn", text: `HTTP ${code} - ${message}` });
  }
  if (location) {
