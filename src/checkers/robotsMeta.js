...
const KNOWN_DIRECTIVES = new Set([
  ...
  'follow', 'nofollow',
  'all', 'none',
  'noarchive', 'nosnippet', 'nosourceinfo',
  ...
  try {
    ...
    if (robotsTags.length === 0) {
      return {
        pass: true,
        found: false,
        message: '로봇 메타 태그 없음 (기본값 index, follow 적용)',
        details: [],
      };
    }
    ...
    for (const tag of robotsTags) {
      ...
      details.push({ type: 'info', text: `<meta name="${tag.name}" content="${tag.content}">` });
